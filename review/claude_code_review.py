...
Использование:
  1) Весь файл:         python claude_code_review.py /path/to/file.py
  2) Выделенный текст:  python claude_code_review.py --code "выделенный код"
  3) Через pipe:        cat file.py | python claude_code_review.py
"""
...
def read_code() -> tuple[str, str]:
    """Читает код из аргументов, файла или stdin. Возвращает (code, filename)."""

    # Вариант 1: --code "текст" (для выделенного фрагмента из IDE)
    if "--code" in sys.argv:
        idx = sys.argv.index("--code")
        code = sys.argv[idx + 1] if idx + 1 < len(sys.argv) else ""
        filename = sys.argv[idx + 2] if idx + 2 < len(sys.argv) else "selection"
        if not code.strip():
            print("No code provided")
            sys.exit(0)
        return code.strip(), filename

    # Вариант 2: путь к файлу
    if len(sys.argv) > 1 and Path(sys.argv[1]).is_file():
        filepath = Path(sys.argv[1])
        return filepath.read_text(encoding="utf-8").strip(), filepath.name

    # Вариант 3: stdin (pipe)
    ...
    print("Использование:")
    print("  python claude_code_review.py /path/to/file.py")
    print('  python claude_code_review.py --code "код" filename')
    print("  cat file.py | python claude_code_review.py")
    sys.exit(1)


def run_with_spinner(prompt: str) -> subprocess.CompletedProcess:
    """Запускает Claude CLI и показывает счётчик секунд."""
...
def save_html_report(review_text: str, filename: str) -> Path:
    """Сохраняет результат ревью в HTML-файл."""
...
def main():
    code, filename = read_code()
    prompt = PROMPT_TEMPLATE.replace("$code", code)

    print(f"Отправляю на ревью: {filename} ({len(code)} символов)")
