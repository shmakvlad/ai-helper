#!/usr/bin/env python3
"""Скрипт для код-ревью через Anthropic API (Claude).

Использование: cat file.py | python ollama_local_review.py [имя_файла]
...
def run_with_spinner(prompt: str) -> str:
    """Отправляет промпт в Claude API и показывает счётчик секунд пока ждём ответ."""
    stop = threading.Event()

    def spinner():
        elapsed = 0
        while not stop.is_set():
            print(f"\rОжидаю ответ от Claude API... {elapsed}с", end="", flush=True)
            time.sleep(1)
            elapsed += 1
...
def main():
    """Точка входа: читает код, отправляет на ревью в Claude API, сохраняет отчёт."""
    filename = sys.argv[1] if len(sys.argv) > 1 else "stdin"
    code = read_code_from_stdin()
    prompt = PROMPT_TEMPLATE.replace("$code", code)

    print("Отправляю код на ревью в Claude API...")
    review_text = run_with_spinner(prompt)

    report = save_html_report(review_text, filename)
